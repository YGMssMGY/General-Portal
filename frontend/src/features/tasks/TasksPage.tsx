		...
		>
			<button
				...
				style={{
					...
					color:
						value === "board" ? "var(--cds-text-primary)" : "var(--cds-text-secondary)",
					cursor: "pointer",
					display: "flex",
					alignItems: "center",
					gap: "0.375rem",
			...
			<button
				...
				style={{
					...
					color:
						value === "list" ? "var(--cds-text-primary)" : "var(--cds-text-secondary)",
					cursor: "pointer",
					display: "flex",
					alignItems: "center",
					gap: "0.375rem",
...
export function TasksPage() {
	...
	async function handleSaveTask(event: FormEvent<HTMLFormElement>) {
		...
		try {
			...
			const msg =
				unknownError instanceof Error ? unknownError.message : "Could not save task";
			setCreateError(msg);
			toast.error(msg);
		} finally {
			setIsCreating(false);
	...
	return (
		<div style={{ position: "relative", minHeight: "calc(100vh - 4rem)" }}>
			...
			<Modal
				...
				description={
					editingTask ? "Update task details." : "Create a task in the workspace."
				}
						...
						<Grid style={{ padding: 0 }}>
							<Column lg={8} md={4} sm={4}>
								<TextInput
									...
									onChange={(e) =>
										setTaskForm((c) => ({ ...c, project: e.target.value }))
									}
								/>
							</Column>
							<Column lg={8} md={4} sm={4}>
