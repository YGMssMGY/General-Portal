...
async function main() {
	...
	const permSets = allRoles.map(
		(r) => ROLE_PERMISSIONS[r as keyof typeof ROLE_PERMISSIONS] || ROLE_PERMISSIONS.member,
	);
			...
			data: [
				{
					...
					budget: 1850,
					summary: "Decor, lighting, and table styling plan for the winter formal venue.",
				},
				{
					workspaceId: workspace.id,
	...
	async function seedEvents() {
		...
		await prisma.eventItem.create({
			data: {
				...
				budgetTotal: 3000,
				owners: {
					create: [{ ownerLabel: "JD" }, { ownerLabel: "AL" }, { ownerLabel: "+3" }],
				},
			},
		});
			...
			data: [
				{
					...
					description: "Our flagship event brought together over 200 participants.",
