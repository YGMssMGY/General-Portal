...
describe("getPermissionsForRole", () => {
  ...
  it("is case-insensitive", () => {
    expect(getPermissionsForRole("ADMIN")).toEqual(ROLE_PERMISSIONS.admin);
    expect(getPermissionsForRole("President")).toEqual(
...
describe("hasAnyPermission", () => {
  it("returns true if user has at least one", () => {
    expect(hasAnyPermission(["task:read"], ["task:read", "task:write"])).toBe(
      true,
    );
  });

  it("returns false if user has none", () => {
    expect(hasAnyPermission(["event:read"], ["task:read", "task:write"])).toBe(
      false,
    );
  });
});
