...
route.get("/search", async (c) => {
	...
	async function searchType(
		...
		try {
			...
			const ors = fields.map((f) => ({ [f]: { contains: query } }));
			const results = await (db as any)[table].findMany({
				where: { workspaceId: wid, OR: ors },
				skip: offset,
				take: effectiveLimit,
	...
	if (!filterType || filterType === "task") {
		queries.push(
			searchType("task", "taskItem", ["title", "project", "assigneeName"], (t: any) => ({
				type: "task" as const,
				id: t.id,
				title: t.title,
	...
	if (!filterType || filterType === "proposal") {
		queries.push(
			searchType("proposal", "proposal", ["title", "submittedBy", "summary"], (p: any) => ({
