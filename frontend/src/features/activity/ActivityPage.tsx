...
export function ActivityPage() {
  ...
  if (error || !data) return <ErrorState message={error ?? "Activity unavailable"} onRetry={refetch} />;
  ...
  return (
    <div>
      <PageHeader title="Activity" description="Track contributions, progress, and workspace changes." />

      <div className="mb-6 grid gap-4 md:grid-cols-3">
        <Card padding="lg">
          <div className="mb-3 flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wide text-text-secondary">Items</span>
        ...
        <Card padding="lg">
          <div className="mb-3 flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wide text-text-secondary">Resources</span>
        ...
        <Card padding="lg">
          <div className="mb-3 flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wide text-text-secondary">Contributors</span>
            ...
            >
              ...
              <div>
                <p className="font-medium text-sm text-text-primary">
                  {item.actorName} <span className="font-normal text-text-secondary">{item.action}</span>
                </p>
                <p className="text-sm text-text-secondary">
                  {item.resourceType}: {item.resourceTitle}
                </p>
                <p className="mt-0.5 text-xs text-text-placeholder">{formatDateTime(item.occurredAt)}</p>
              </div>
            </div>
          ))}
        </div>
