...
route.post("/proposals/:id/approve", async (c) => {
  ...
  if (!allowedRoles)
    return c.json({ error: "Proposal is already fully approved" }, 400);
  ...
  const history: any[] = proposal.approvalHistory
    ? JSON.parse(proposal.approvalHistory)
    : [];
  history.push({
    step: nextStep,
    approverId: userId,
  ...
  if (nextStep !== "approved") {
    ...
    const submitter = await db.userAccount.findFirst({
