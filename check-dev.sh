...
test_port() {
    local name="$1" port="$2" required="${3:-true}"
    local open=false
    ...
    if $open; then
        ...
        echo "INFO $name is not listening on port $port (optional)"
    fi
}
...
test_http() {
    ...
    if command -v curl &>/dev/null; then
        ...
        echo "INFO curl/wget not found, skipping HTTP check for $name"
    fi
}

test_port "PostgreSQL" 5432 false
test_port "Redis (optional)" 6379 false
test_http "Backend (Hono)" "http://localhost:3001/api/health"
test_http "Frontend (Vite)" "http://localhost:5173"
